poisson_model <- function(){
  ...
  for(j in 1:length(age.group)) {
    ...
    sink(sprintf("output/PoiReg_Stepwise_summary_%s_v6.txt",age2))
    print(summary(step.model))
    sink()
    write.csv(summary(step.model)$coefficients,sprintf("output/PoiReg_Stepwise_summary_%s_v6.csv",age2))
    ...
    write.csv(pred.dat,sprintf("output/Prediction_Age_%s_PoiReg_Stepwise_v6.csv",age2))

  }
  RMSE.output<-cbind(age.group,RMSE)
  RMSE.output
  write.csv(RMSE.output,"output/PoiReg_Stepwise_RMSE_v6.csv")
  return(RMSE.output)
}
