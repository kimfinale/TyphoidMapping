predict_loglinear_model <- function(){
  ...
  for(j in 1:length(age.group)) {
    age<-age.group[j]
    if(j<=3) {age2<-age;cov.name2<-read.csv(sprintf("output/LReg_Stepwise_summary_%s_v6.csv",age2))[-1,1]}
    if(j==4) { age2<-"over 14y";cov.name2<-read.csv(sprintf("output/LReg_Stepwise_summary_%s_v6.csv",age2))[-1,1] }
    ...
    write.csv(pred2,sprintf("output/Forecasting/Forecasting_Age_%s_LReg_Stepwise_v6.csv",age2),row.names=FALSE)
    ...
    for(i in c("pred.o","lower.o","upper.o","pred","lower","upper")) {
      ...
      writeRaster(pred.df.raster,file=sprintf("output/Forecasting/Forecasting_%s_Age_%s_LReg_Stepwise_v6.tif",i,age2),overwrite=TRUE)
    }
  }
