negbin_model_1 <- function(){
  ...
  for(j in 1:length(age.group)) {
    ...
    sink(sprintf("output/NBReg_Stepwise_summary_%s_v6.txt",age2))
    print(summary(step.model))
    sink()
    write.csv(summary(step.model)$coefficients,sprintf("output/NBReg_Stepwise_summary_%s_v6.csv",age2))
    ...
    round(pred.dat)
    write.csv(pred.dat,sprintf("output/Prediction_Age_%s_NBReg_Stepwise_v6.csv",age2))
